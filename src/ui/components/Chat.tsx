...
async function readAgentsInstructions(plugin: GeminiHelperPlugin): Promise<string | null> {
	const agentsFile = plugin.app.vault.getFileByPath("AGENTS.md");
	if (!agentsFile) {
		return null;
	}
	try {
		return await plugin.app.vault.cachedRead(agentsFile);
	} catch {
		return null;
	}
}
...
const Chat = forwardRef<ChatRef, ChatProps>(({ plugin }, ref) => {
	...
	const isCliMode = isCodexCliMode;

	const isConfigReady = codexCliVerified;

	const allowWebSearch = false;
	const allowRag = false;
	...
	const availableModels = [CODEX_CLI_MODEL];
	...
	useEffect(() => {
		const handleRagSettingChanged = (name: string | null) => {
			setSelectedRagSetting(name && name !== "__websearch__" ? null : null);
		};
	...
	useEffect(() => {
		if (currentModel !== "codex-cli") {
			setCurrentModel("codex-cli");
			void plugin.selectModel("codex-cli");
		}
	...
	const sendMessageViaCli = async (content: string, attachments?: Attachment[], skillPath?: string) => {
		...
		try {
			...
			const cliName = isCodexCli ? "Codex CLI" : isClaudeCli ? "Claude CLI" : "Gemini CLI";
			const agentsInstructions = await readAgentsInstructions(plugin);
			let systemPrompt = "You are a Codex-style assistant integrated with Obsidian.";
			systemPrompt += `\n\nThe active Obsidian vault is your only workspace boundary. Treat it as the current project and never assume access outside it.`;
...
The active Obsidian vault is your only workspace boundary. Use vault-relative paths, preserve note conventions, and provide clear, concise responses. When working with notes, keep changes explicit and approval-oriented.`;
			...
			{isConfigReady ? (
				...
				<div className="gemini-helper-config-required">
					<div className="gemini-helper-config-message">
						...
						<p>Open plugin settings to configure the Codex executable path.</p>
					</div>
				</div>
			)}
		</div>
