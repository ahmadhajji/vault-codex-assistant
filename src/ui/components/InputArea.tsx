...
const InputArea = forwardRef<InputAreaHandle, InputAreaProps>(function InputArea({
  ...
  const showVaultControls = allowWebSearch || ragEnabled || mcpServers.length > 0 || !vaultToolModeOnlyNone;
  const showSecondarySelectors = availableModels.length > 1 || allowWebSearch || ragEnabled;
    ...
    <div className={`gemini-helper-input-container ${isCollapsed ? "collapsed" : ""}`}>
      ...
      {!isCollapsed && (
        ...
        <div className="gemini-helper-input-buttons">
          ...
          {showVaultControls && <div className="gemini-helper-vault-tool-container" ref={vaultToolMenuRef}>
            <button
      ...
      {!isCollapsed && showSecondarySelectors && (
